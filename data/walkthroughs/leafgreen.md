# Pokémon LeafGreen Walkthrough

<!-- DB refs: item_availability, tm_move, hm_move, shop_item, tutor_move, encounter -->
<!-- Version: leafgreen (id 11), version_group_id 7 -->

## Badge 0: Starting Out
- **Starter**: Bulbasaur / Charmander / Squirtle — Pallet Town, Professor Oak.
- Bulbasaur dominates Brock + Misty. Charmander struggles early. Squirtle balanced.
- **LeafGreen exclusives**: Sandshrew (Rt 4), Vulpix (Rt 7-8, Pokémon Mansion), Bellsprout (Rt 5-6, 12-15, 24-25), Pinsir (Safari Zone), Slowpoke (Rt 12, Cape), Staryu (Rt 19-20 Super Rod), Magmar (Pokémon Mansion).

### Items & TMs
| Location | Items |
|---|---|
| Pallet Town | Potion (from PC) |
| Route 1 | Potion (hidden) |
| Viridian City | Potion (from Mart clerk after Parcel delivery) |
| Route 22 | — |
| Viridian Forest | Potion, Antidote, Poké Ball (hidden), Potion (hidden) |

### Key Items
| Item | Given By | Location | Prerequisite |
|---|---|---|---|
| Pokédex + 5 Poké Balls | Professor Oak | Pallet Town lab | Deliver Oak's Parcel from Viridian Mart |

### Unlocks
- Deliver Oak's Parcel (Viridian Mart → Oak) → Pokédex + 5 Poké Balls.
- Pokédex required to buy Poké Balls at any Mart.
- Route 22 rival fight is optional pre-Brock XP.

### Gates
- **Route 3**: Blocked by gate NPC until Boulder Badge obtained — Mt. Moon and Cerulean City unreachable.
- **Viridian Gym**: No leader present; locked until all 7 other badges are obtained.
- **Route 22 west dead-end**: Open for early rival XP only; no passage beyond until all 8 badges (Victory Road).

### Side Quests
- **Oak's Aide — Route 2 gate (after Cut)**: Catch 10 Pokémon → HM05 Flash.

---

## Badge 1: Brock (Boulder Badge)
- **Gym**: Pewter City. Rock-type.
- **Team**: Geodude (Lv 12), Onix (Lv 14).
- **Rec level**: 12-14.
- **Threats**: Onix — high Def, Rock Tomb (lowers Speed).
- **Strategy**: Water/Grass. Butterfree Confusion. Mankey (Rt 22) Fighting.
- **Badge**: Lv 20 obedience. Flash outside battle.

### Items & TMs
| Location | Items |
|---|---|
| Pewter City | — |
| Pewter Gym | TM39 Rock Tomb (Brock reward) |
| Route 3 | — |
| Mt. Moon | TM09 Bullet Seed (1F), TM46 Thief (B2F), Moon Stone (B2F), Rare Candy (hidden, B2F), Star Piece (hidden, B2F) |
| Mt. Moon | Dome Fossil OR Helix Fossil (choose one from Super Nerd) |

### Key Items
| Item | Given By | Location | Prerequisite |
|---|---|---|---|
| Dome Fossil OR Helix Fossil | Super Nerd (choose one) | Mt. Moon B2F | Defeat Super Nerd |

### Unlocks
- Boulder Badge → Flash usable outside battle.
- Route 3 opens after Brock.
- Mt. Moon exit → Route 4 → Cerulean City.

### Gates
- **Return to Pewter City (from Cerulean side)**: No hard lock — Mt. Moon is always traversable on foot in both directions. Fast travel via Fly unavailable until HM02 Fly + Thunder Badge (Badge 3 phase).
- **Pewter Museum back entrance**: Blocked by Cut tree — Old Amber unobtainable until HM01 Cut (Badge 2 era).
- **Cerulean Gym / story events**: Physically reachable after Mt. Moon, but Misty must be beaten to progress further east.

---

## Badge 2: Misty (Cascade Badge)
- **Gym**: Cerulean City. Water-type.
- **Team**: Staryu (Lv 18), Starmie (Lv 21).
- **Rec level**: 18-21.
- **Threats**: Starmie — fast, Water Pulse (confuse), Swift.
- **Strategy**: Pikachu (ThunderShock). Bellsprout (LG exclusive, Rt 24-25). Ivysaur resists.
- **Badge**: Lv 30 obedience. Cut outside battle.

### Items & TMs
| Location | Items |
|---|---|
| Route 4 | TM05 Roar (hidden), Magikarp (purchase, $500 from man in Pokémon Center) |
| Cerulean City | Bicycle (Bike Voucher from Vermilion) |
| Route 24 (Nugget Bridge) | Nugget (reward after 5 trainers) |
| Route 25 | TM43 Secret Power |
| Cerulean City | TM28 Dig (from Rocket grunt backyard after Nugget Bridge) |
| Cerulean Gym | TM03 Water Pulse (Misty reward) |

### Key Items
| Item | Given By | Location | Prerequisite |
|---|---|---|---|
| S.S. Ticket | Bill | Bill's house, Route 25 | Beat Nugget Bridge trainers + visit Bill |

### Unlocks
- Cascade Badge → Cut outside battle.
- Beat Nugget Bridge + visit Bill → S.S. Ticket.
- S.S. Ticket → board S.S. Anne in Vermilion.
- Cut → Rt 9 access (east of Cerulean). Cut tree also opens Rt 2 backpath to Oak's Aide.
- Burgled house (Cerulean) → chase Rocket grunt → TM28 Dig.

### Gates
- **S.S. Anne [ONE-WAY]**: Ship departs permanently once HM01 Cut is received from the captain — cannot reboard. Collect TM31 Brick Break (1F) and TM44 Rest (B1F) before leaving.
- **Route 9 / Rock Tunnel**: Cut-tree gate; inaccessible until HM01 Cut obtained this phase.
- **Return to Pewter City**: Foot travel via Mt. Moon only — fast travel (Fly) not available until Thunder Badge + HM02 Fly (Badge 3 phase).

---

## Badge 3: Lt. Surge (Thunder Badge)
- **Gym**: Vermilion City. Electric-type. Trash can puzzle.
- **Team**: Voltorb (Lv 21), Pikachu (Lv 18), Raichu (Lv 24).
- **Rec level**: 22-25.
- **Threats**: Raichu — Shock Wave (never misses), Double Team.
- **Strategy**: Ground immune. Diglett/Dugtrio (Diglett's Cave Rt 11) perfect.
- **Badge**: Fly outside battle.

### Items & TMs
| Location | Items |
|---|---|
| Vermilion City | Old Rod (Fishing Guru, house south of Pokémon Center), Vs. Seeker (from girl in Pokémon Center), Bike Voucher (Pokémon Fan Club chairman) |
| S.S. Anne | HM01 Cut (Captain's cabin), TM31 Brick Break (1F cabin), TM44 Rest (B1F cabin) |
| Route 11 | — |
| Diglett's Cave | Connects Rt 11 ↔ Rt 2 |
| Route 9 | TM40 Aerial Ace |
| Route 10 | — |
| Vermilion Gym | TM34 Shock Wave (Lt. Surge reward) |

### Key Items
| Item | Given By | Location | Prerequisite |
|---|---|---|---|
| Old Rod | Fishing Guru | House south of Pokémon Center, Vermilion City | Talk to NPC |
| Vs. Seeker | Girl in Pokémon Center | Vermilion City Pokémon Center | Talk to NPC |
| Bike Voucher | Pokémon Fan Club Chairman | Pokémon Fan Club, Vermilion City | Listen to his story |
| HM01 Cut | S.S. Anne Captain | S.S. Anne Captain's cabin, Vermilion City | Board S.S. Anne with S.S. Ticket |

### Unlocks
- HM01 Cut (S.S. Anne) → cut trees everywhere. S.S. Anne departs after Cut obtained — do not leave without it.
- Thunder Badge → Fly outside battle.
- Cut → Rt 9 → Rock Tunnel → Lavender Town.
- Diglett's Cave (Rt 11) → Rt 2 south side → Oak's Aide (Flash).
- Bike Voucher (Vermilion Fan Club) → Bicycle (Cerulean Bike Shop).

### Gates
- **Pewter City fast travel (resolved)**: HM02 Fly (Rt 16, requires Cut) + Thunder Badge unlocks fast travel to all previously visited cities, including Pewter — Mt. Moon backtrack no longer needed.
- **S.S. Anne**: Permanently closed — no further access.
- **Lavender Town / Celadon / Fuchsia**: Foot travel required until Fly obtained this badge phase.

### Side Quests
- **Oak's Aide — Rt 2 gate**: Catch 10 Pokémon → HM05 Flash.

---

## Badge 4: Erika (Rainbow Badge)
- **Gym**: Celadon City. Grass-type.
- **Team**: Victreebel (Lv 29), Tangela (Lv 24), Vileplume (Lv 29).
- **Rec level**: 28-30.
- **Threats**: Victreebel — Acid, Razor Leaf. Vileplume — Stun Spore, Petal Dance.
- **Strategy**: Fire, Ice, Flying, Psychic. Any Fire-type sweeps.
- **Badge**: Strength outside battle.

### Items & TMs
| Location | Items |
|---|---|
| Rock Tunnel | — (bring Flash) |
| Lavender Town | — (Pokémon Tower blocked without Silph Scope) |
| Celadon Dept Store 2F | TM05 Roar (buy) |
| Celadon Dept Store 3F | TM15 Hyper Beam (7500), TM16 Light Screen (3000), TM17 Protect (3000), TM20 Safeguard (3000), TM33 Reflect (3000), TM42 Facade (3000), TM48 Skill Swap (3000) |
| Celadon Dept Store 4F | Poké Doll (1000), Fire Stone, Thunder Stone, Water Stone, Leaf Stone |
| Celadon Condominiums | TM41 Torment (from woman, 1F), Eevee (gift, rooftop back entrance) |
| Celadon Game Corner | Coin Case (from man in back), TM13 Ice Beam (4000 coins), TM23 Iron Tail (3500 coins), TM35 Flamethrower (4000 coins), TM24 Thunderbolt (4000 coins) |
| Celadon Game Corner Prizes | Abra (120 coins), Clefairy (750), Pinsir (2500, LG), Dratini (4600), Porygon (6500) |
| Rocket Hideout (B1F-B4F) | TM12 Taunt (B2F), Rare Candy (B3F), TM49 Snatch (B4F), Silph Scope (Giovanni, B4F), Lift Key (B4F) |
| Celadon City | TM45 Attract (from girl, 1F house) |
| Celadon Gym | TM19 Giga Drain (Erika reward) |

### Key Items
| Item | Given By | Location | Prerequisite |
|---|---|---|---|
| Eevee | NPC gift | Celadon Condominiums rooftop (back entrance) | None |
| Coin Case | Man in back room | Celadon Game Corner | Talk to NPC |
| Silph Scope | Giovanni (defeat him) | Rocket Hideout B4F, Celadon City | Clear Rocket Hideout |
| Poké Flute | Mr. Fuji | Lavender Town (after Pokémon Tower rescue) | Silph Scope + clear Pokémon Tower |

### Unlocks
- Rainbow Badge → Strength outside battle.
- Silph Scope (Rocket Hideout B4F, beat Giovanni) → Pokémon Tower accessible.
- Pokémon Tower: rescue Mr. Fuji → Poké Flute.
- Poké Flute → wake Snorlax (Rt 12, Rt 16). Two available.
- Vending machine drinks (Celadon roof) → give to Saffron guards → Saffron City access.

### Gates
- **Pokémon Tower upper floors**: Ghost Marowak blocks progress without Silph Scope — clear Rocket Hideout first.
- **Saffron City**: All 4 gate guards blocked until one vending machine drink given (Fresh Water / Soda Pop / Lemonade from Celadon Dept Store roof); one drink opens all gates permanently.
- **Route 12 / Route 16**: Snorlax physically blocks the road until Poké Flute used.

### Side Quests
- **Oak's Aide — Rt 11 gate 2F**: Catch 30 Pokémon → Itemfinder.

---

## Badge 5: Koga (Soul Badge)
- **Gym**: Fuchsia City. Poison-type. Invisible walls.
- **Team**: Koffing (Lv 37), Muk (Lv 39), Koffing (Lv 37), Weezing (Lv 43).
- **Rec level**: 38-43.
- **Threats**: Weezing Self-Destruct. Muk Minimize + Sludge.
- **Strategy**: Psychic or Ground. Alakazam or Dugtrio.
- **Badge**: Surf outside battle.

### Items & TMs
| Location | Items |
|---|---|
| Route 12 | TM36 Sludge Bomb, Snorlax encounter (Lv 30, use Poké Flute), Leftovers (hidden, where Snorlax slept) |
| Route 16 | HM02 Fly (from girl in house, Cut tree), Snorlax encounter (Lv 30) |
| Safari Zone Area 1 | TM11 Sunny Day |
| Safari Zone Area 2 | TM47 Steel Wing |
| Safari Zone Center | HM03 Surf (Secret House, deep in zone) |
| Safari Zone Area 3 | TM22 Solar Beam, TM32 Double Team (hidden), Gold Teeth |
| Fuchsia City | Good Rod (Fishing Guru), HM04 Strength (Safari Zone Warden, give Gold Teeth) |
| Route 15 | TM18 Rain Dance |
| Fuchsia Gym | TM06 Toxic (Koga reward) |

### Key Items
| Item | Given By | Location | Prerequisite |
|---|---|---|---|
| Good Rod | Fishing Guru | Fuchsia City (house east of Pokémon Center) | Talk to NPC |
| HM03 Surf | NPC in Secret House | Safari Zone (deep inside, past all areas) | Pay 500 to enter Safari Zone |
| Gold Teeth | Ground item | Safari Zone Area 3 | Enter Safari Zone |
| HM04 Strength | Safari Zone Warden | Fuchsia City (Warden's house) | Give him the Gold Teeth from Safari Zone |
| HM02 Fly | Girl in house | Route 16 (south of Celadon, cut tree to reach) | HM01 Cut |
| Exp. Share | Oak's Aide | Route 15 gate 2F | Catch 50 Pokémon |
| Amulet Coin | Oak's Aide | Route 16 gate 2F | Catch 40 Pokémon |
| Super Rod | Fishing Guru | Route 12 (house south of Lavender Town) | Talk to NPC |

### Unlocks
- Soul Badge → Surf outside battle (requires HM03 from Safari Zone).
- Gold Teeth (Safari Zone Area 3) → give to Warden → HM04 Strength.
- HM02 Fly (Rt 16 house, need Cut) → fly to visited cities.
- Surf → new water routes accessible (Rt 19-21, Seafoam Islands, Cinnabar, Power Plant).
- Poké Flute wakes both Snorlax (Rt 12/16) — catch or defeat to open routes.

### Gates
- **Water routes / Cinnabar Island / Power Plant**: Locked behind Surf + Soul Badge — all inaccessible before this phase.
- **Seafoam Islands deep floors (Articuno / HM07 Waterfall)**: Require Surf and Strength boulder-puzzle solutions; cannot be cleared without both HMs.
- **Safari Zone Secret House (HM03 Surf)**: Must be reached within the 500-step limit of a single Safari Zone visit — plan route before entering.

### Side Quests
- **Oak's Aide — Rt 15 gate 2F**: Catch 50 Pokémon → Exp. Share.
- **Oak's Aide — Rt 16 gate 2F**: Catch 40 Pokémon → Amulet Coin.

---

## Badge 6: Sabrina (Marsh Badge)
- **Gym**: Saffron City. Psychic-type. Teleport pad puzzle.
- **Team**: Kadabra (Lv 38), Mr. Mime (Lv 37), Venomoth (Lv 38), Alakazam (Lv 43).
- **Rec level**: 40-43.
- **Threats**: Alakazam — max Speed + Psychic STAB.
- **Strategy**: Bug, Ghost, Dark moves. Snorlax tanks. Shadow Ball on anything.
- **Badge**: Rock Smash outside battle.

### Items & TMs
| Location | Items |
|---|---|
| Saffron City | TM29 Psychic (from Mr. Psychic, house SE) |
| Silph Co. 2F | — |
| Silph Co. 3F | TM01 Focus Punch |
| Silph Co. 5F | Card Key (opens all locked doors), TM36 Sludge Bomb |
| Silph Co. 7F | TM08 Bulk Up |
| Silph Co. 7F | Lapras (gift, Lv 25, from Silph employee), Rival battle |
| Silph Co. 11F | Master Ball (from President after defeating Giovanni) |
| Fighting Dojo | Hitmonlee OR Hitmonchan (beat all trainers, choose one) |
| Saffron City | TM04 Calm Mind (from Sabrina after gym win) |

### Key Items
| Item | Given By | Location | Prerequisite |
|---|---|---|---|
| Card Key | Found on floor | Silph Co. 5F, Saffron City | Access Silph Co. (need drink for guards) |
| Lapras | Silph Co. employee | Silph Co. 7F, Saffron City | Reach 7F during Silph Co. event |
| Master Ball | Silph Co. President | Silph Co. 11F, Saffron City | Defeat Giovanni at Silph Co. |
| Hitmonlee OR Hitmonchan | Karate Master (choose one) | Fighting Dojo, Saffron City | Beat all Dojo trainers |

### Unlocks
- Saffron City access requires giving a drink to any gate guard (Celadon vending machines: Fresh Water, Soda Pop, or Lemonade). One drink opens all 4 gates.
- Card Key (Silph Co. 5F) → opens locked doors on all Silph floors.
- Clear Silph Co. (beat Giovanni 11F) → Saffron City freed from Rockets.
- Marsh Badge → Rock Smash outside battle.

### Gates
- **Silph Co. upper-floor rooms**: Individual doors locked until Card Key (5F) obtained — floors accessible on foot but most rooms blocked.
- **Cerulean Cave**: Locked until post-E4 + National Dex (60+ species registered).
- **Fighting Dojo prize**: Hitmonlee / Hitmonchan withheld until all Dojo trainers are defeated.

### Tutor Moves (version_group 7)
| Location | Move |
|---|---|
| Rt 4 exit (Mt. Moon) | Mega Punch, Mega Kick |
| Pewter Museum (back door, Cut) | Seismic Toss |
| Viridian City (behind Cut tree) | Dream Eater |
| Rock Tunnel 1F | Rock Slide |
| Silph Co. (with Card Key) | Thunder Wave |
| Saffron City (Copycat's house, give Poké Doll) | Mimic |
| Pokémon Mansion (Cinnabar) | Metronome |
| Victory Road exit | Double-Edge |

---

## Badge 7: Blaine (Volcano Badge)
- **Gym**: Cinnabar Island. Fire-type. Quiz doors.
- **Team**: Growlithe (Lv 42), Ponyta (Lv 40), Rapidash (Lv 42), Arcanine (Lv 47).
- **Rec level**: 43-47.
- **Threats**: Arcanine — Fire Blast + Intimidate.
- **Strategy**: Water, Ground, Rock. Surf trivializes.
- **Badge**: All Pokémon obey.

### Items & TMs
| Location | Items |
|---|---|
| Route 19-20 | — (Surf route to Seafoam/Cinnabar) |
| Seafoam Islands | HM07 Waterfall (deep B4F), Articuno (static, Lv 50, requires Strength puzzles) |
| Power Plant (Rt 10, Surf) | TM25 Thunder, Zapdos (static, Lv 50), Electrode (static, Lv 34 ×2 disguised as items) |
| Pokémon Mansion 1F | — |
| Pokémon Mansion 2F | — |
| Pokémon Mansion 3F | Secret Key, TM14 Blizzard (hidden) |
| Pokémon Mansion B1F | TM22 Solar Beam |
| Cinnabar Lab | Fossil revival: Dome Fossil → Kabuto (Lv 5), Helix Fossil → Omanyte (Lv 5), Old Amber → Aerodactyl (Lv 5) |
| Cinnabar Island | TM38 Fire Blast (from Blaine after gym win) |

### Key Items
| Item | Given By | Location | Prerequisite |
|---|---|---|---|
| HM07 Waterfall | Found on ground | Seafoam Islands B4F | Surf + Strength to navigate ice puzzles |
| Secret Key | Found in mansion | Pokémon Mansion 3F, Cinnabar Island | Navigate mansion switch puzzles |
| Old Amber | Museum Scientist | Pewter Museum backroom | HM01 Cut (cut tree to enter) |

### Unlocks
- Surf from Pallet Town south or Fuchsia Rt 19 south → Cinnabar Island.
- Surf + Strength → Seafoam Islands (Rt 20) → Articuno + HM07 Waterfall.
- Surf from Cerulean Rt 10 → Power Plant → Zapdos + TM25 Thunder.
- Secret Key (Pokémon Mansion 3F) → unlocks Cinnabar Gym door.
- Old Amber (Pewter Museum backroom, requires Cut) → revive Aerodactyl at Cinnabar Lab.
- Volcano Badge → all Pokémon obey.

### Gates
- **Cinnabar Gym**: Locked until Secret Key found in Pokémon Mansion 3F — navigate mansion switch puzzles to reach it.
- **Power Plant**: Requires Surf from Rt 10 (north of Cerulean, past Rock Tunnel) or approached from Lavender side.
- **Navel Rock / Birth Island**: Event-only locations (Mystic Ticket / Aurora Ticket); inaccessible in standard retail play.

---

## Badge 8: Giovanni (Earth Badge)
- **Gym**: Viridian City. Ground-type. Spin tile puzzle.
- **Team**: Rhyhorn (Lv 45), Dugtrio (Lv 42), Nidoqueen (Lv 44), Nidoking (Lv 45), Rhyhorn (Lv 50).
- **Rec level**: 45-50.
- **Threats**: Nidoking/Nidoqueen — Earthquake. Rhyhorn Lv 50 bulk.
- **Strategy**: Water, Grass, Ice. Lapras (Surf + Ice Beam) ideal. Flying dodges EQ.
- **Badge**: All HMs usable.

### Items & TMs
| Location | Items |
|---|---|
| Viridian City | TM26 Earthquake (from Giovanni after gym win), Macho Brace (hidden, tile where Giovanni stood) |
| Victory Road 1F | TM02 Dragon Claw, TM07 Hail |
| Victory Road 2F | TM50 Overheat, Full Heal, Rare Candy (hidden) |
| Victory Road 3F | TM37 Sandstorm |
| Route 23 | — (badge gates check all 8 badges) |
| Cerulean Cave (post-E4) | Mewtwo (static, Lv 70), TM04 Calm Mind (B1F), TM26 Earthquake (B1F) |

### Unlocks
- Earth Badge → all HMs usable outside battle.
- Route 22 → Route 23 → Victory Road. Each badge gate on Rt 23 checks one badge.
- Victory Road requires Surf, Strength, Rock Smash to traverse.
- After E4: Cerulean Cave opens (requires National Dex) → Mewtwo.

### Gates
- **Victory Road (Route 23 checkpoints)**: Eight sequential badge-gate NPCs — each checks one specific badge; all 8 must be held to pass through.
- **Viridian Gym**: Now unlocked (Giovanni appears) — was inaccessible since game start.
- **Cerulean Cave**: Remains locked even post-E4 until National Dex obtained (60+ species registered).

---

## Elite Four
- **Victory Road**: Requires Surf, Strength, Rock Smash.
- **Rec level**: 55-60+.
- **Lorelei** (Ice): Dewgong (52), Cloyster (51), Slowbro (52), Jynx (54), Lapras (54). → Electric, Fighting.
- **Bruno** (Fighting): Onix (51), Hitmonchan (53), Hitmonlee (53), Onix (54), Machamp (56). → Psychic, Flying.
- **Agatha** (Ghost/Poison): Gengar (54), Golbat (54), Haunter (53), Arbok (56), Gengar (58). → Psychic, Ground.
- **Lance** (Dragon): Gyarados (56), Dragonair (54), Dragonair (54), Aerodactyl (58), Dragonite (60). → Ice Beam sweeps.
- **Champion** (Rival): Pidgeot, Alakazam, Rhydon + starter-dependent trio. Diverse coverage needed.

### Post-E4 Unlocks
- Complete E4 → Bill takes you to One Island → Tri-Pass (Islands 1-3).
- National Dex (60+ caught species + beat E4) → Cerulean Cave opens → Mewtwo.
- Rainbow Pass (Islands 4-7) → complete Ruby gem quest on One Island + get National Dex.
- Ruby gem + Sapphire gem → give both to Celio → Network Machine → trade with Ruby/Sapphire/Emerald.

### Gates
- **Cerulean Cave**: Locked until National Dex obtained (60+ species); completing the E4 alone is insufficient.
- **Sevii Islands 4-7**: Locked behind Rainbow Pass — requires Ruby gem delivered to Celio + National Dex.
- **Rocket Warehouse (Five Island)**: Locked until both passwords collected (one from Gideon on Five Island, one from Rocket grunts at Mt. Ember).
- **Dotted Hole (Six Island)**: Requires HM01 Cut on the sealed front door — inaccessible without it.

---

## Sevii Islands

### Progression
1. Beat E4 → Bill takes you to One Island (Tri-Pass: Islands 1-3).
2. Find Ruby gem (Mt. Ember, One Island) → give to Celio.
3. Get National Dex (60+ species in Kanto Dex) → Celio gives Rainbow Pass (Islands 4-7).
4. Find Sapphire gem (Dotted Hole, Six Island) → Gideon steals it.
5. Clear Rocket Warehouse (Five Island) → recover Sapphire from Gideon.
6. Give Sapphire to Celio → Network Machine complete → Hoenn trading enabled.

### One Island (Knot Island)
| Location | Items / Events |
|---|---|
| Pokémon Center | Celio's Network Machine (gem quest starts here) |
| Ember Spa (Kindle Road) | HM06 Rock Smash (from old man) |
| Kindle Road | — (trainers, wild Ponyta, Geodude) |
| Mt. Ember exterior | Moltres (static, Lv 50) |
| Mt. Ember interior | Ruby gem (behind 2 Rocket grunts, post-E4). Requires Strength + Rock Smash |

### Two Island (Boon Island)
| Location | Items / Events |
|---|---|
| Joyful Game Corner | Move tutors: Body Slam, Double-Edge, Counter, Mega Kick, Seismic Toss, Mimic, Swords Dance (cost: 2 Tiny Mushrooms or 1 Big Mushroom each) |
| Cape Brink | Ultimate starter moves: Frenzy Plant (Venusaur), Blast Burn (Charizard), Hydro Cannon (Blastoise). Requires max friendship |
| Two Island town | Lostelle's father (starts rescue quest → Berry Forest on Three Island) |

### Three Island (Kin Island)
| Location | Items / Events |
|---|---|
| Berry Forest | Rescue Lostelle (find her at forest end, return to Two Island father) |
| Bond Bridge | Trainers. Wild Pidgey, Pidgeotto, Bellsprout (LG), Venonat |
| Three Isle Port | Biker gang battle (clear to access Three Isle Path post-National Dex) |
| Three Isle Path | Dunsparce (rare, post-National Dex) |

### Four Island (Floe Island) — requires Rainbow Pass
| Location | Items / Events |
|---|---|
| Daycare Center | Pokémon breeding available. Two Pokémon at once |
| Icefall Cave | Wild Lapras (B1F, rare), Lorelei event (help her fight Team Rocket). Requires Surf + Waterfall |
| Icefall Cave back | HM07 Waterfall (if not obtained at Seafoam). Delibird, Sneasel, Seel |

### Five Island (Chrono Island) — requires Rainbow Pass
| Location | Items / Events |
|---|---|
| Five Isle Meadow | — (trainers) |
| Memorial Pillar | Memorial for lost Onix. Hidden item |
| Water Labyrinth | Trainers (Surf required) |
| Resort Gorgeous | High-level trainers, Selphy's butler gives rewards after Lost Cave |
| Lost Cave | Maze dungeon. Find Selphy → she goes to Resort Gorgeous → gives rare items for showing her specific Pokémon |
| Rocket Warehouse | Passwords required (1st from Gideon on Five Island, 2nd from Rocket grunts at Mt. Ember). Beat Scientist Gideon → recover Sapphire gem. TM36 Sludge Bomb (if not got earlier) |

### Six Island (Fortune Island) — requires Rainbow Pass
| Location | Items / Events |
|---|---|
| Altering Cave | Wild Zubat (event-only alternates never distributed) |
| Green Path | Water route, trainers |
| Water Path | Wild Psyduck, Golduck. Trainers |
| Ruin Valley | Entrance to Dotted Hole. Wild Arbok (FR) / Sandslash (LG), Marowak, Wobbuffet |
| Dotted Hole | Use Cut on front door. Navigate by reading Braille directions (up/left/right/down). Sapphire gem at end → Gideon steals it |
| Pattern Bush | Wild Caterpie, Metapod, Weedle, Kakuna, Ledyba, Spinarak, Heracross |
| Outcast Island | Trainers |

### Seven Island (Quest Island) — requires Rainbow Pass
| Location | Items / Events |
|---|---|
| Trainer Tower | 4 challenge modes (Single, Double, Knockout, Mixed). Time-based rewards |
| Sevault Canyon | High-level trainers. Wild Larvitar (rare), Phanpy, Cubone, Marowak |
| Tanoby Key | Sevault Canyon side area. Solve boulder puzzle (Strength required) → unlocks Unown in Tanoby Chambers |
| Tanoby Chambers (7 rooms) | Unown (28 forms: A-Z, ! , ?). Each chamber has different letter sets |

### Sevii Island Key Items
| Item | Location | Purpose |
|---|---|---|
| Tri-Pass | Bill (after E4) | Access Islands 1-3 |
| Rainbow Pass | Celio (after Ruby gem + National Dex) | Access Islands 4-7 |
| Ruby gem | Mt. Ember interior (beat 2 Rockets) | Give to Celio |
| Sapphire gem | Dotted Hole → Rocket Warehouse | Give to Celio → enables Hoenn trades |

### Sevii Island Tutors (version_group 7)
| Location | Move | Cost |
|---|---|---|
| Two Island (Cape Brink) | Frenzy Plant / Blast Burn / Hydro Cannon | Free (fully evolved starter, max friendship) |
| Two Island (Joyful Game Corner) | Body Slam | 2 Tiny Mushroom or 1 Big Mushroom |
| Two Island (Joyful Game Corner) | Double-Edge | 2 Tiny Mushroom or 1 Big Mushroom |
| Two Island (Joyful Game Corner) | Counter | 2 Tiny Mushroom or 1 Big Mushroom |
| Two Island (Joyful Game Corner) | Mega Kick | 2 Tiny Mushroom or 1 Big Mushroom |
| Two Island (Joyful Game Corner) | Seismic Toss | 2 Tiny Mushroom or 1 Big Mushroom |
| Two Island (Joyful Game Corner) | Mimic | 2 Tiny Mushroom or 1 Big Mushroom |
| Two Island (Joyful Game Corner) | Swords Dance | 2 Tiny Mushroom or 1 Big Mushroom |

---

## Static Encounters Summary
| Pokémon | Location | Level | Requirement |
|---|---|---|---|
| Snorlax ×2 | Rt 12, Rt 16 | 30 | Poké Flute |
| Articuno | Seafoam Islands B4F | 50 | Surf + Strength |
| Zapdos | Power Plant | 50 | Surf (from Rt 10) |
| Moltres | Mt. Ember (One Island) | 50 | Post-E4, Sevii pass |
| Mewtwo | Cerulean Cave B1F | 70 | Post-E4 + National Dex |
| Entei | Roaming Kanto | 50 | Post-E4, starter = Bulbasaur |
| Raikou | Roaming Kanto | 50 | Post-E4, starter = Squirtle |
| Suicune | Roaming Kanto | 50 | Post-E4, starter = Charmander |
| Lugia | Navel Rock | 70 | Mystic Ticket event |
| Ho-Oh | Navel Rock | 70 | Mystic Ticket event |
| Deoxys | Birth Island | 30 | Aurora Ticket event |

---

---

## HM Checklist
| HM | Move | Location | Badge Required |
|---|---|---|---|
| HM01 | Cut | S.S. Anne Captain | Cascade (Badge 2) |
| HM02 | Fly | Rt 16 house (Cut tree) | Thunder (Badge 3) |
| HM03 | Surf | Safari Zone Secret House | Soul (Badge 5) |
| HM04 | Strength | Fuchsia Warden (Gold Teeth) | Rainbow (Badge 4) |
| HM05 | Flash | Oak's Aide Rt 2 (10 caught) | Boulder (Badge 1) |
| HM06 | Rock Smash | Ember Spa (One Island) / Rt 12 | Marsh (Badge 6) |
| HM07 | Waterfall | Seafoam Islands B4F | — (FRLG field use: post-E4 only) |

---

---

## All TMs (01–50)
| TM | Move | Type | Location | Method | Earliest |
|---|---|---|---|---|---|
| TM01 | Focus Punch | Fighting | Silph Co. 3F | Floor item | Badge 6 |
| TM02 | Dragon Claw | Dragon | Victory Road 1F | Floor item | Badge 8 |
| TM03 | Water Pulse | Water | Cerulean Gym (Misty reward) | Gym reward | Badge 2 |
| TM04 | Calm Mind | Psychic | Saffron Gym (Sabrina reward) | Gym reward | Badge 6 |
| TM05 | Roar | Normal | Rt 4 | Hidden item | Badge 1 |
| TM06 | Toxic | Poison | Fuchsia Gym (Koga reward) | Gym reward | Badge 5 |
| TM07 | Hail | Ice | Victory Road 1F | Floor item | Badge 8 |
| TM08 | Bulk Up | Fighting | Silph Co. 7F | Floor item | Badge 6 |
| TM09 | Bullet Seed | Grass | Mt. Moon 1F | Floor item | Badge 1 |
| TM10 | Hidden Power | Normal | Pickup ability / Dept Store lottery | — | Varies |
| TM11 | Sunny Day | Fire | Safari Zone Area 1 | Floor item | Badge 5 |
| TM12 | Taunt | Dark | Rocket Hideout B2F | Floor item | Badge 4 |
| TM13 | Ice Beam | Ice | Celadon Game Corner | 4000 coins | Badge 4 |
| TM14 | Blizzard | Ice | Pokémon Mansion 3F | Hidden item | Badge 7 |
| TM15 | Hyper Beam | Normal | Celadon Dept Store 3F | Buy (7500₽) | Badge 4 |
| TM16 | Light Screen | Psychic | Celadon Dept Store 3F | Buy (3000₽) | Badge 4 |
| TM17 | Protect | Normal | Celadon Dept Store 3F | Buy (3000₽) | Badge 4 |
| TM18 | Rain Dance | Water | Rt 15 | Floor item | Badge 5 |
| TM19 | Giga Drain | Grass | Celadon Gym (Erika reward) | Gym reward | Badge 4 |
| TM20 | Safeguard | Normal | Celadon Dept Store 3F | Buy (3000₽) | Badge 4 |
| TM21 | Frustration | Normal | Rocket Game Corner / Silph Co. | NPC gift | Badge 4 |
| TM22 | Solar Beam | Grass | Pokémon Mansion B1F | Floor item | Badge 7 |
| TM23 | Iron Tail | Steel | Celadon Game Corner | 3500 coins | Badge 4 |
| TM24 | Thunderbolt | Electric | Celadon Game Corner | 4000 coins | Badge 4 |
| TM25 | Thunder | Electric | Power Plant (Rt 10, Surf required) | Floor item | Badge 5 |
| TM26 | Earthquake | Ground | Viridian Gym (Giovanni reward) | Gym reward | Badge 8 |
| TM27 | Return | Normal | Rt 12 / CellPokemon Fan Club | NPC gift | Badge 3 |
| TM28 | Dig | Ground | Cerulean City — Rocket grunt north side | NPC gift | Badge 2 |
| TM29 | Psychic | Psychic | Mr. Psychic's house, Saffron City | NPC gift | Badge 6 |
| TM30 | Shadow Ball | Ghost | Celadon Game Corner | 4500 coins | Badge 4 |
| TM31 | Brick Break | Fighting | S.S. Anne 1F (room) | Floor item | Badge 2 |
| TM32 | Double Team | Normal | Safari Zone Area 3 | Hidden item | Badge 5 |
| TM33 | Reflect | Psychic | Celadon Dept Store 3F | Buy (3000₽) | Badge 4 |
| TM34 | Shock Wave | Electric | Vermilion Gym (Lt. Surge reward) | Gym reward | Badge 3 |
| TM35 | Flamethrower | Fire | Celadon Game Corner | 4000 coins | Badge 4 |
| TM36 | Sludge Bomb | Poison | Rt 12 | Floor item | Badge 5 |
| TM37 | Sandstorm | Rock | Victory Road 3F | Floor item | Badge 8 |
| TM38 | Fire Blast | Fire | Cinnabar Gym (Blaine reward) | Gym reward | Badge 7 |
| TM39 | Rock Tomb | Rock | Pewter Gym (Brock reward) | Gym reward | Badge 1 |
| TM40 | Aerial Ace | Flying | Rt 9 | Floor item | Badge 3 |
| TM41 | Torment | Dark | Celadon Condominiums 1F | NPC gift | Badge 4 |
| TM42 | Facade | Normal | Celadon Dept Store 3F | Buy (3000₽) | Badge 4 |
| TM43 | Secret Power | Normal | Rt 25 | Floor item | Badge 2 |
| TM44 | Rest | Psychic | S.S. Anne B1F (cabin) | Floor item | Badge 2 |
| TM45 | Attract | Normal | Celadon City 1F house | NPC gift | Badge 4 |
| TM46 | Thief | Dark | Mt. Moon B2F | Floor item | Badge 1 |
| TM47 | Steel Wing | Steel | Safari Zone Area 2 | Floor item | Badge 5 |
| TM48 | Skill Swap | Psychic | Celadon Dept Store 3F | Buy (3000₽) | Badge 4 |
| TM49 | Snatch | Dark | Rocket Hideout B4F | Floor item | Badge 4 |
| TM50 | Overheat | Fire | Victory Road 2F | Floor item | Badge 8 |
