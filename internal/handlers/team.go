...
func UpdateTeam(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if formIDStr == "" {
			...
			db.Exec(`UPDATE run_pokemon SET in_party = 0, party_slot = NULL WHERE run_id = ? AND party_slot = ? AND in_party = 1`, run.ID, slot) //nolint:errcheck
...
func EvolveBox(db *sql.DB, pokeClient *pokeapi.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if versionGroupID > 0 {
			go pokeClient.EnsurePokemon(db, toFormID, versionGroupID)
		}
