...
// A custom connector ensures all pool connections receive per-connection PRAGMAs,
// enabling concurrent reads while background data hydration writes proceed.
func Open(dbPath string) (*sql.DB, error) {
	...
	tmp, err := sql.Open("sqlite", dbPath)
	if err != nil {
	...
	db := sql.OpenDB(&pragmaConnector{dsn: dbPath, drv: drv})
