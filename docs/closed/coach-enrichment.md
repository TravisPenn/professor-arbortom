...
- [x] All 324 type matchups (18×18) are encoded. Values verified against Bulbapedia Gen 3+ chart.
- [x] `TypeEffectiveness("fire", "grass")` returns `2.0`.
- [x] `TypeEffectiveness("normal", "ghost")` returns `0.0`.
- [x] `DualTypeEffectiveness("ground", "fire", "flying")` returns `1.0` (2.0 × 0.5, cancels).
- [x] Unit tests cover super effective, not very effective, immune, and dual-type combinations.
...
- [x] A team of 3 Water types reports Ground and Electric as `Weaknesses`.
- [x] A team with no Fighting-type moves reports `Gaps` includes "normal", "dark", "steel", etc.
- [x] `CounterChain("electric", waterTeam, 2)` returns Ground at depth 1, Water/Grass/Ice at depth 2.
- [x] Analysis runs in <1ms for a 6-member team (in-memory computation, no DB).
- [x] Unit tests with known team compositions and expected profiles.
...
- [x] `FindEvolutionPaths` for Eevee (form 133) returns paths to all 3 Gen 3 Eeveelutions.
- [x] Umbreon path is marked `BlockedBy: "missing: high friendship"` if flag is unset.
- [x] With `no_trade_evolutions` active, Alakazam path shows `BlockedBy: "no_trade_evolutions"`.
- [x] `MoveDelayAnalysis` for Pikachu → Raichu flags Thunderbolt (Pikachu Lv26, Raichu never).
- [x] BFS terminates cleanly on circular or missing edges (no infinite loops).
- [x] Unit tests for linear chains (Charmander→Charmeleon→Charizard), branching (Eevee), and blocked paths.
...
- [x] After seeding Charizard (form 6): `pokemon_type` has `(6,1,"fire")` and `(6,2,"flying")`.
- [x] After seeding Charizard: `pokemon_stats` has `(6, 78, 84, 78, 109, 85, 100)`.
- [x] After seeding Charizard: `pokemon_ability` has `(6,1,"blaze")`.
- [x] Existing PokeAPI cache prevents re-fetching — only processes forms not yet populated.
- [x] Migration is safe to run against existing databases (empty tables, no data loss).
...
- [x] A party Pokémon can have 1–4 moves assigned via the team page.
- [x] Assigning a 5th move prompts a "replace which move?" selection.
- [x] Coach payload includes `current_moves` with `move_id`, `name`, and `type_name` for each slot.
- [x] Existing runs with no move data assigned continue to work (empty table, no errors).
...
- [x] Coach payload size stays under 32 KB for a 6-member team (measure with `json.Marshal`).
- [x] `team_analysis` is populated when at least one party member has type data seeded.
- [x] `evolution_paths` is populated for each party member with at least one evolution edge.
- [x] `party_details[].current_moves` is populated only for Pokémon with assigned moves.
- [x] Existing Coach queries continue to work if no COACH-004/005 data exists (graceful omission).
- [x] Move `power`, `accuracy`, `pp` are included in `party_moves` entries.
...
- [x] ZeroClaw system prompt distinguishes grounded vs. general-knowledge answers.
- [x] `context_note` is populated on every Coach request.
- [x] Coach correctly answers "How do I breed Dragon Dance onto Larvitar?" using LLM knowledge.
- [x] Coach correctly answers "What are my team's weaknesses?" using computed type analysis.
