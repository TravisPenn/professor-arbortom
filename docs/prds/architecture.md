...
│     │  HTTP :8000
│     ▼
└── LXC 131: pokemonprofessor-lxc (192.168.1.131)
      │
      │  /usr/local/bin/pokemonprofessor   ← single static Go binary
      │  /naspool/appdata/pokemonprofessor/db/pokemon.sqlite
      │  /naspool/appdata/pokemonprofessor/config/.env
      │
      │  ZEROCLAW_GATEWAY (optional, blank = disabled)
      │     │
      │     ▼ HTTP :42617
      │  LXC 130: zeroclaw-lxc (192.168.1.130)   ← separate PRD, deploy independently
...
| Bind mount | Proxmox `pct` mp0 in LXC config | `/naspool/appdata/pokemonprofessor` available at the same path inside LXC |
...
```
scp pokemonprofessor-linux-amd64 root@192.168.1.100:/tmp/
...
`/naspool/appdata/zeroclaw/config/config.toml`. It is appended by the Proxmox deploy playbook,
